export type Lang = "en" | "ur";

/**
 * UI strings. Keep keys flat and stable — Urdu copy needs a human review pass
 * before pilot (tracked in the sprint doc).
 */
const en = {
  appName: "Zebra",
  tagline: "Safe stories & learning for kids",
  heroLine: "Stories, spellings, maths and good morals — in Urdu and English.",
  getStarted: "Get Started",
  kidMode: "Kid Mode",
  forParents: "For Parents",
  language: "اردو",

  // Onboarding
  obWelcome: "Welcome to Zebra!",
  obFamilyName: "What should we call your family?",
  obFamilyPlaceholder: "e.g. The Ahmed Family",
  obPickLanguage: "App language",
  obSetPin: "Set your parent PIN",
  obPinHint: "4 digits. Kids will need this PIN to leave Kid Mode — don't share it!",
  obConfirmPin: "Enter the PIN again to confirm",
  obPinMismatch: "PINs don't match — try again",
  obFirstChild: "Add your first child",
  obChildName: "Child's name (or nickname)",
  obAgeBand: "Age",
  obStoryLanguage: "Story language",
  obFinish: "All set — let's go!",
  next: "Next",
  back: "Back",

  // Age bands
  band35: "3–5 years",
  band68: "6–8 years",
  band912: "9–12 years",
  english: "English",
  urdu: "Urdu",

  // Parental gate
  gateTitle: "Parents only",
  gateAsk: "Enter your PIN to continue",
  gateWrong: "That PIN isn't right",
  cancel: "Cancel",

  // Parent dashboard
  pdTitle: "Parent Dashboard",
  pdChildren: "Children",
  pdAddChild: "Add child",
  pdNoChildren: "No child profiles yet — add one to unlock Kid Mode.",
  pdLibrary: "Library",
  pdLibraryCount: "stories ready to read",
  pdComingSoon: "Coming next",
  pdSoonCreate: "Create a personalized AI story (with your approval gate)",
  pdSoonAudio: "Audio narration in Urdu & English",
  pdSoonGames: "Spelling & maths games",
  pdLock: "Lock & exit",
  delete: "Remove",
  save: "Save",

  // Kid mode
  kidWho: "Who is reading today?",
  kidHello: "Hello",
  kidPickStory: "Pick a story!",
  kidNoStories: "No stories for this profile yet.",
  kidExit: "Exit",
  kidTheEnd: "The End!",
  kidGreatJob: "Great job reading!",
  kidReadAgain: "Read again",
  kidAllStories: "All stories",

  // Themes
  themeSharing: "Sharing",
  themeHonesty: "Honesty",
  themeCounting: "Counting",
  themeCourage: "Courage",
  themeKindness: "Kindness",

  // Characters
  charZee: "Zee 🦓",
  charMano: "Mano 🐐",
  charSitara: "Sitara 🐆",
  charDada: "Dada Kachwa 🐢",

  // Create studio (adults only)
  pdCreateSection: "Create a story with AI",
  pdReviewSection: "Review & approve",
  csForChild: "For which child?",
  csTheme: "Lesson",
  csCharacter: "Main character",
  csLength: "Length",
  csShort: "Short",
  csMedium: "Longer",
  csChildName: "Feature this name (optional)",
  csGenerate: "Create story",
  csGenerating: "Creating your story…",
  csNeedChild: "Add a child profile first, then you can create stories for them.",
  csMadeWith: "Made with",
  csGenGemini: "AI (Gemini)",
  csGenTemplate: "Zebra story templates",

  // Review queue + approval gate
  rqEmpty: "No stories waiting — create one above.",
  rqWaiting: "Waiting for your approval",
  rqPreview: "Preview",
  rqHide: "Hide",
  rqApprove: "Approve",
  rqReject: "Reject",
  rqApproved: "Approved — your child can read it now!",
  rqBlocked: "Our safety check stopped this story. Please try creating again.",
  rqModPassed: "Passed the automated safety check. Your approval is the final gate.",
  rqSafetyNote: "Nothing reaches your child until you approve it.",
  rqForChild: "For",

  // Narration
  listen: "Listen",
  stopAudio: "Stop",

  // Activities hub (Kid Mode)
  kidPlay: "Let's play!",
  kidStories: "Stories",
  kidSpell: "Spelling",
  kidMaths: "Maths",
  kidColor: "Coloring",
  kidStars: "stars",

  // Games — shared
  gCorrect: "Correct!",
  gWellDone: "Well done!",
  gTryAgain: "Try again",
  gNext: "Next",
  gRoundDone: "Round complete!",
  gStarsEarned: "stars earned",
  gPlayAgain: "Play again",
  gScore: "Score",

  // Spelling
  spYourTurn: "Spell the word!",
  spHint: "Tap the letters in order",
  spHearWord: "Hear the word",

  // Maths
  gmHowMany: "How many?",
  gmWhichBigger: "Which is bigger?",
  gmPickAnswer: "Pick the answer",

  // Coloring
  colPick: "Pick a page to color",
  colChoose: "Choose a color",
  colUndo: "Undo",
  colClear: "Clear",
  colSave: "Save",
  colSaved: "Saved to your gallery!",
  colGallery: "My Gallery",
  colEmpty: "Your saved pictures will appear here.",

  // School / teacher workspace
  teacherTools: "Teacher tools",
  schoolTitle: "Teacher Tools",
  schoolSub: "Make lessons, worksheets, and classes — present or print.",
  schoolPresent: "Presentation maker",
  schoolPresentSub: "Build a class slideshow",
  schoolWorksheet: "Worksheets",
  schoolWorksheetSub: "Printable spelling & maths",
  schoolClasses: "Classes",
  schoolClassesSub: "Organize your students",

  // Presentation maker
  pmTitle: "Presentation maker",
  pmReady: "Ready lessons",
  pmReadyHint: "Start from a complete, editable lesson",
  pmNewTopic: "Make from your own topic",
  pmTopic: "Topic",
  pmTopicHint: "e.g. The Solar System",
  pmSlides: "Slides",
  pmGenerate: "Create slides",
  pmGenerating: "Building your slides…",
  pmYourDecks: "Your presentations",
  pmNoDecks: "No presentations yet — make one above.",
  pmBlocked: "Our safety check stopped that topic. Please try another.",
  pmPresent: "Present",
  pmPrint: "Print / Save PDF",
  pmEdit: "Edit",
  pmDone: "Done",
  pmAddPoint: "Add point",
  pmSlideLabel: "Slide",
  pmExit: "Exit",

  // Worksheets
  wsTitle: "Worksheet maker",
  wsType: "Type",
  wsSpelling: "Spelling list",
  wsMaths: "Maths drills",
  wsCount: "How many",
  wsGenerate: "Make worksheet",
  wsShowAnswers: "Show answer key",
  wsWriteWord: "Write the word",
  wsSolve: "Solve",
  wsName: "Name",
  wsDate: "Date",

  // Classes
  clTitle: "Classes",
  clNewClass: "New class",
  clName: "Class name",
  clNamePlaceholder: "e.g. Grade 2 — Section A",
  clGrade: "Grade",
  clCreate: "Create class",
  clCode: "Share code",
  clShareHint: "Students join with this code",
  clNoClasses: "No classes yet — create your first.",

  // Auth (cloud mode)
  email: "Email",
  password: "Password",
  signIn: "Sign in",
  signOut: "Sign out",
  createAccount: "Create your account",
  obAccount: "First, create your parent account",
  haveAccount: "Already have an account?",
  needAccount: "New to Zebra?",
  authError: "Couldn't do that — check your email and password",
  weakPassword: "Password must be at least 6 characters",
  confirmEmail: "Almost there! Check your email to confirm, then sign in.",
  loading: "Please wait…",
} as const;

export type UiKey = keyof typeof en;

const ur: Record<UiKey, string> = {
  appName: "زیبرا",
  tagline: "بچوں کے لیے محفوظ کہانیاں اور تعلیم",
  heroLine: "کہانیاں، ہجے، حساب اور اچھے اخلاق — اردو اور انگریزی میں۔",
  getStarted: "شروع کریں",
  kidMode: "بچوں کا حصہ",
  forParents: "والدین کے لیے",
  language: "English",

  obWelcome: "زیبرا میں خوش آمدید!",
  obFamilyName: "آپ کی فیملی کا نام کیا رکھیں؟",
  obFamilyPlaceholder: "مثلاً احمد فیملی",
  obPickLanguage: "ایپ کی زبان",
  obSetPin: "اپنا والدین والا پن سیٹ کریں",
  obPinHint: "4 ہندسے۔ بچوں کے حصے سے نکلنے کے لیے یہی پن چاہیے ہوگا — کسی کو نہ بتائیں!",
  obConfirmPin: "تصدیق کے لیے پن دوبارہ لکھیں",
  obPinMismatch: "پن ایک جیسے نہیں — دوبارہ کوشش کریں",
  obFirstChild: "اپنا پہلا بچہ شامل کریں",
  obChildName: "بچے کا نام",
  obAgeBand: "عمر",
  obStoryLanguage: "کہانی کی زبان",
  obFinish: "سب تیار — چلیں!",
  next: "آگے",
  back: "پیچھے",

  band35: "3 سے 5 سال",
  band68: "6 سے 8 سال",
  band912: "9 سے 12 سال",
  english: "انگریزی",
  urdu: "اردو",

  gateTitle: "صرف والدین کے لیے",
  gateAsk: "آگے بڑھنے کے لیے اپنا پن لکھیں",
  gateWrong: "یہ پن درست نہیں",
  cancel: "منسوخ",

  pdTitle: "والدین کا ڈیش بورڈ",
  pdChildren: "بچے",
  pdAddChild: "بچہ شامل کریں",
  pdNoChildren: "ابھی کوئی پروفائل نہیں — بچوں کا حصہ کھولنے کے لیے ایک شامل کریں۔",
  pdLibrary: "لائبریری",
  pdLibraryCount: "کہانیاں پڑھنے کے لیے تیار",
  pdComingSoon: "آگے کیا آرہا ہے",
  pdSoonCreate: "اپنے بچے کے لیے ذاتی AI کہانی (آپ کی منظوری کے بعد)",
  pdSoonAudio: "اردو اور انگریزی میں آواز کے ساتھ کہانیاں",
  pdSoonGames: "ہجے اور حساب کے کھیل",
  pdLock: "بند کر کے باہر جائیں",
  delete: "حذف کریں",
  save: "محفوظ کریں",

  kidWho: "آج کون پڑھے گا؟",
  kidHello: "السلام علیکم",
  kidPickStory: "کوئی کہانی چنو!",
  kidNoStories: "اس پروفائل کے لیے ابھی کہانیاں نہیں۔",
  kidExit: "باہر",
  kidTheEnd: "ختم شد!",
  kidGreatJob: "واہ! بہت خوب پڑھا!",
  kidReadAgain: "دوبارہ پڑھو",
  kidAllStories: "ساری کہانیاں",

  themeSharing: "بانٹنا",
  themeHonesty: "سچائی",
  themeCounting: "گنتی",
  themeCourage: "ہمت",
  themeKindness: "مہربانی",

  charZee: "زی 🦓",
  charMano: "مانو 🐐",
  charSitara: "ستارہ 🐆",
  charDada: "دادا کچھوا 🐢",

  pdCreateSection: "AI سے کہانی بنائیں",
  pdReviewSection: "جائزہ اور منظوری",
  csForChild: "کس بچے کے لیے؟",
  csTheme: "سبق",
  csCharacter: "مرکزی کردار",
  csLength: "لمبائی",
  csShort: "مختصر",
  csMedium: "لمبی",
  csChildName: "یہ نام شامل کریں (اختیاری)",
  csGenerate: "کہانی بنائیں",
  csGenerating: "آپ کی کہانی بن رہی ہے…",
  csNeedChild: "پہلے ایک بچے کی پروفائل شامل کریں، پھر آپ ان کے لیے کہانیاں بنا سکتے ہیں۔",
  csMadeWith: "بنائی گئی",
  csGenGemini: "AI (جیمنی)",
  csGenTemplate: "زیبرا کہانی سانچے",

  rqEmpty: "کوئی کہانی منتظر نہیں — اوپر سے ایک بنائیں۔",
  rqWaiting: "آپ کی منظوری کا منتظر",
  rqPreview: "جھلک دیکھیں",
  rqHide: "چھپائیں",
  rqApprove: "منظور کریں",
  rqReject: "مسترد کریں",
  rqApproved: "منظور ہو گئی — اب آپ کا بچہ پڑھ سکتا ہے!",
  rqBlocked: "ہمارے حفاظتی جائزے نے یہ کہانی روک دی۔ براہِ کرم دوبارہ بنائیں۔",
  rqModPassed: "خودکار حفاظتی جائزہ پاس کر لیا۔ آپ کی منظوری آخری مرحلہ ہے۔",
  rqSafetyNote: "جب تک آپ منظوری نہ دیں، کچھ بھی بچے تک نہیں پہنچتا۔",
  rqForChild: "کے لیے",

  listen: "سنیں",
  stopAudio: "روکیں",

  kidPlay: "آؤ کھیلیں!",
  kidStories: "کہانیاں",
  kidSpell: "ہجے",
  kidMaths: "حساب",
  kidColor: "رنگ بھرو",
  kidStars: "ستارے",

  gCorrect: "درست!",
  gWellDone: "شاباش!",
  gTryAgain: "دوبارہ کوشش کرو",
  gNext: "اگلا",
  gRoundDone: "راؤنڈ مکمل!",
  gStarsEarned: "ستارے ملے",
  gPlayAgain: "دوبارہ کھیلو",
  gScore: "اسکور",

  spYourTurn: "لفظ کے ہجے کرو!",
  spHint: "حروف ترتیب سے دبائیں",
  spHearWord: "لفظ سنیں",

  gmHowMany: "کتنے ہیں؟",
  gmWhichBigger: "کون سا بڑا ہے؟",
  gmPickAnswer: "جواب چنو",

  colPick: "رنگ بھرنے کے لیے صفحہ چنو",
  colChoose: "رنگ چنو",
  colUndo: "واپس",
  colClear: "صاف کرو",
  colSave: "محفوظ کرو",
  colSaved: "آپ کی گیلری میں محفوظ ہو گیا!",
  colGallery: "میری گیلری",
  colEmpty: "آپ کی محفوظ تصویریں یہاں نظر آئیں گی۔",

  teacherTools: "اساتذہ کے اوزار",
  schoolTitle: "اساتذہ کے اوزار",
  schoolSub: "اسباق، ورک شیٹس اور کلاسیں بنائیں — پیش کریں یا پرنٹ کریں۔",
  schoolPresent: "پریزنٹیشن میکر",
  schoolPresentSub: "کلاس کے لیے سلائیڈ شو بنائیں",
  schoolWorksheet: "ورک شیٹس",
  schoolWorksheetSub: "ہجے اور حساب کی پرنٹ ایبل شیٹس",
  schoolClasses: "کلاسیں",
  schoolClassesSub: "اپنے طلبہ کو منظم کریں",

  pmTitle: "پریزنٹیشن میکر",
  pmReady: "تیار اسباق",
  pmReadyHint: "ایک مکمل، قابلِ ترمیم سبق سے شروع کریں",
  pmNewTopic: "اپنے موضوع سے بنائیں",
  pmTopic: "موضوع",
  pmTopicHint: "مثلاً نظامِ شمسی",
  pmSlides: "سلائیڈز",
  pmGenerate: "سلائیڈز بنائیں",
  pmGenerating: "آپ کی سلائیڈز بن رہی ہیں…",
  pmYourDecks: "آپ کی پریزنٹیشنز",
  pmNoDecks: "ابھی کوئی پریزنٹیشن نہیں — اوپر سے ایک بنائیں۔",
  pmBlocked: "ہمارے حفاظتی جائزے نے یہ موضوع روک دیا۔ براہِ کرم کوئی اور آزمائیں۔",
  pmPresent: "پیش کریں",
  pmPrint: "پرنٹ / PDF محفوظ کریں",
  pmEdit: "ترمیم",
  pmDone: "ہو گیا",
  pmAddPoint: "نکتہ شامل کریں",
  pmSlideLabel: "سلائیڈ",
  pmExit: "باہر",

  wsTitle: "ورک شیٹ میکر",
  wsType: "قسم",
  wsSpelling: "ہجے کی فہرست",
  wsMaths: "حساب کی مشق",
  wsCount: "کتنے",
  wsGenerate: "ورک شیٹ بنائیں",
  wsShowAnswers: "جوابات دکھائیں",
  wsWriteWord: "لفظ لکھیں",
  wsSolve: "حل کریں",
  wsName: "نام",
  wsDate: "تاریخ",

  clTitle: "کلاسیں",
  clNewClass: "نئی کلاس",
  clName: "کلاس کا نام",
  clNamePlaceholder: "مثلاً جماعت دوم — سیکشن اے",
  clGrade: "جماعت",
  clCreate: "کلاس بنائیں",
  clCode: "شیئر کوڈ",
  clShareHint: "طلبہ اس کوڈ سے شامل ہوتے ہیں",
  clNoClasses: "ابھی کوئی کلاس نہیں — اپنی پہلی بنائیں۔",

  email: "ای میل",
  password: "پاس ورڈ",
  signIn: "سائن اِن",
  signOut: "سائن آؤٹ",
  createAccount: "اپنا اکاؤنٹ بنائیں",
  obAccount: "پہلے، اپنا والدین اکاؤنٹ بنائیں",
  haveAccount: "پہلے سے اکاؤنٹ ہے؟",
  needAccount: "زیبرا پر نئے ہیں؟",
  authError: "نہیں ہو سکا — اپنا ای میل اور پاس ورڈ دیکھیں",
  weakPassword: "پاس ورڈ کم از کم 6 حروف کا ہو",
  confirmEmail: "بس تھوڑا سا باقی! تصدیق کے لیے اپنا ای میل دیکھیں، پھر سائن اِن کریں۔",
  loading: "انتظار کریں…",
};

export const dictionaries: Record<Lang, Record<UiKey, string>> = { en, ur };
