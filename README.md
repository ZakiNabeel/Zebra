...
- **2026-06-16 — Sprint 5 shipped:** school workspace (the schools-first wedge) — a **Teacher Tools** area with a bilingual **presentation maker** (ready lessons or AI/$0-scaffold custom topics → edit → full-screen present → **PDF export**), a printable **worksheet maker** (spelling + maths, reusing the game engines), and **classes** with share codes. PDF export uses the browser's own print engine and tools persist locally — **zero backend cost, works offline**. ([report](docs/sprints/sprint-05-school-workspace.md) · [diagrams](docs/sprints/sprint-05-sequence-diagrams.md)).
- Next: Sprint 6 — offline/PWA + frontend redesign polish.
